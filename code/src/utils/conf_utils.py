...
def _parse_completed_ft_epochs(ls_result: List[str]) -> List[int]:
    epochs = []
    for line in ls_result[1:]:
        if not line.strip():
            continue
        try:
            epochs.append(int(line.split(",", 1)[0]))
        except (IndexError, ValueError):
            continue
    return epochs


def _load_completed_ft_epochs(output_dir: str) -> List[int]:
    fn_result = os.path.join(output_dir, "result.csv")
    if not os.path.exists(fn_result):
        return []
    with open(fn_result, "r") as fp:
        return _parse_completed_ft_epochs(fp.readlines())


def get_latest_completed_ft_ckp(
    misc_utils,
    output_dir: str,
    eval_only: bool = False,
) -> Tuple[str, Optional[int]]:
    if eval_only:
        return misc_utils.get_latest_ckp(output_dir, eval_only)

    epochs = _load_completed_ft_epochs(output_dir)
    for epoch in reversed(epochs):
        ckp = os.path.join(output_dir, f"epoch_{epoch}")
...
def _trim_ft_result_to_epoch(ls_result: List[str], epoch: int) -> List[str]:
    if not ls_result or epoch is None:
        return ls_result
    trimmed = [ls_result[0]]
    for line in ls_result[1:]:
        if not line.strip():
...
def _trim_csv_to_global_step(
    ...
    for line in lines[1:]:
        ...
        try:
            global_step = int(parts[global_step_col])
        except (IndexError, ValueError):
            continue
...
def _last_int_from_csv(lines: List[str], col_idx: int) -> Optional[int]:
    if not lines or len(lines) == 1:
        return None
    try:
        return int(lines[-1].strip().split(",")[col_idx])
    except (IndexError, ValueError):
        return None
...
def init_log_conf_for_ft(
    ...
    if pretrain_cpt == output_dir:
        ...
        _, prev_epoch = get_latest_completed_ft_ckp(
            misc_utils, pretrain_cpt, eval_only
        )
        last_step_index = (prev_epoch + 1) * steps_per_epoch

        if not eval_only:
            ls_result = _trim_ft_result_to_epoch(ls_result, prev_epoch)
            ls_log = _trim_csv_to_global_step(ls_log, 2, last_step_index)
            ls_loss = _trim_csv_to_global_step(ls_loss, 2, last_step_index)

        last_step_index_from_log = _last_int_from_csv(ls_log, 2)
        if not eval_only and last_step_index_from_log is not None:
            assert (
                last_step_index >= last_step_index_from_log
            ), f"last_step_index: {last_step_index}, last_step_index_from_log: {last_step_index_from_log}"

        last_step_index_from_result = _last_int_from_csv(ls_result, 1)
        if not eval_only and last_step_index_from_result is not None:
            assert (
        ...
        last_step_index_from_loss = _last_int_from_csv(ls_loss, 2)
        if not eval_only and last_step_index_from_loss is not None:
            assert (
