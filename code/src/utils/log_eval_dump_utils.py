...
def log_dump_ft_training_stats(
    ...
    skip_test_during_train = (
        train_cfg.ft_eval.skip_test_during_train
        and not train_cfg.ft_eval.eval_only
    )
    ...
    best_improved = False
    if train_cfg.ft_eval.save_best_by_valid and val_ogb_eval_res is not None:
        if train_stats.best_eval_res is None:
            best_improved = True
            train_stats.best_eval_res = val_ogb_eval_res
        else:
            best_improved, train_stats.best_eval_res = metrics_utils.compare_metrics_res(
                val_ogb_eval_res, train_stats.best_eval_res
            )
        if best_improved:
            train_stats.best_epoch = epoch
            train_stats.epochs_since_best = 0
            if not train_cfg.ft_eval.eval_only:
                print(
                    f"[{datetime.now()}] New best valid result at epoch {epoch}: "
                    f"{train_stats.best_eval_res}. Saving epoch_best checkpoint."
                )
                misc_utils.save_ckp(
                    output_dir,
                    model,
            ...
            train_stats.epochs_since_best += 1
            patience = train_cfg.ft_eval.early_stopping_patience
            if patience is not None and train_stats.epochs_since_best >= patience:
                train_stats.should_stop = True
                print(
                    f"[{datetime.now()}] Early stopping triggered after "
                    f"{train_stats.epochs_since_best} epochs without valid improvement."
                )
    ...
    if skip_test_during_train:
        print(
            f"[{datetime.now()}][end of epoch {epoch}][local {i}][global {j}] "
            "Skip test evaluation during training; final test runs from epoch_best."
        )
        test_loss, test_cls_metrics, test_ogb_eval_res, test_triplet = (
            None,
            None,
            None,
            None,
    ...
    test_details = (
        test_cls_metrics.results_in_details() if test_cls_metrics is not None else ""
    )
    print(
        f"[{datetime.now()}][epoch {epoch}][local {i}][global {j}] train_loss: {tr_loss}, "
        f"valid_loss: {val_loss}, test_loss: {test_loss}, {test_details},\n"
    ...
    val_metric_str = (
        ",".join(val_cls_metrics.results_in_str_tuple())
        if val_cls_metrics is not None
        else ""
    )
    test_metric_str = (
        ",".join(test_cls_metrics.results_in_str_tuple())
        if test_cls_metrics is not None
        else ""
    )
    train_stats.ls_log.append(
