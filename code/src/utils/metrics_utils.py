...
class MultiLabelClassificationMetrics:
    def __init__(self, device, num_labels=2, **kwargs):
        ...
        self.num_labels = num_labels
        self.auroc_metric = (
            metrics.BinaryAUROC(num_tasks=num_labels).to(device)
            if num_labels == 1
            else None
        )
        self.auroc_vec = None
        ...
        self.ls_labels = []
        self.ls_idx = []
        self.has_single_label_updates = False

    def update(self, logits, labels, idx):
        if self.num_labels == 1:
            y_pred = logits.float().sigmoid().reshape(-1).to(self.device)
            y_true = labels.reshape(-1).to(self.device)
            is_labeled = y_true == y_true
            if is_labeled.any():
                self.auroc_metric.update(y_pred[is_labeled], y_true[is_labeled])
                self.has_single_label_updates = True
    ...
    def compute(self):
        if self.num_labels == 1:
            if self.has_single_label_updates:
                self.auroc_vec = np.atleast_1d(
                    self.auroc_metric.compute().cpu().numpy()
                )  # [num_labels]
                self.auroc_mean = self.auroc_vec.mean()  # scalar
            else:
                self.auroc_vec = np.array([np.nan])
                self.auroc_mean = np.nan
            return

        logits = torch.vstack(self.ls_logits).float().sigmoid().detach().cpu().numpy()
        labels = torch.vstack(self.ls_labels).detach().cpu().numpy()
        auroc_vec = []
        for label_idx in range(self.num_labels):
            task_labels = labels[:, label_idx]
            task_scores = logits[:, label_idx]
            is_labeled = ~np.isnan(task_labels)
            task_labels = task_labels[is_labeled]
            task_scores = task_scores[is_labeled]
            if (
                task_labels.size == 0
                or np.sum(task_labels == 1) == 0
                or np.sum(task_labels == 0) == 0
            ):
                auroc_vec.append(np.nan)
            else:
                auroc_vec.append(roc_auc_score(task_labels, task_scores))
        self.auroc_vec = np.asarray(auroc_vec, dtype=np.float64)
        valid = np.isfinite(self.auroc_vec)
        self.auroc_mean = float(self.auroc_vec[valid].mean()) if valid.any() else np.nan
